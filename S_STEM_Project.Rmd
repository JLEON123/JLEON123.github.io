...
CleanTempChi <- broom::tidy(TempChi)
CleanPrecipChi <- broom::tidy(PrecipChi)
CleanFertChi <- broom::tidy(FertChi)

Temp1 <- rbind(CleanTempChi, CleanPrecipChi)
FullChiSq <- rbind(Temp1, CleanFertChi)
FullChiSq <- as.data.frame(FullChiSq)
Data <- c("Plant Outcome & Temperature", "Plant Outcome & Precipitation", "Plant Outcome & Fertilizer")
row.names(FullChiSq) <- Data
CleanFullChiSq <- FullChiSq %>% 
  select(1:3) %>% 
  rename(
...
<p style="text-align:center;">**Package Versions**</p>
```{r, programtable}
Pversions <- read.csv("./data/S_Stem_project/program_metadata/All_Pversions.csv")

Pversions %>% 
  kable() %>% 
  kable_styling(font_size = 35) %>% 
  kable_classic_2(lightable_options = c("hover", "condensed", "striped")) %>% 
  scroll_box(height = "3in")
```

### *Scripts: 00_Remove_Primers.R & 01_Process_Raw_16S_Reads.R*
