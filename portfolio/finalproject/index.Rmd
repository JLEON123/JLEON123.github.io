...
Library <- c("R", "tidyverse", "dada2", "purrr", "Biostrings", "ShortRead", "phyloseq", "ggmap", 'kableExtra')
Version <- c("4.1.1", '1.3.2', '1.20.0', '0.3.5', '2.60.2', '1.50.0', '1.36.0', '3.0.1', '1.3.4')
...
otu_table(Df) %>% rowSums()
# how many times was each taxon observed?
otu_table(Df) %>% colSums()

# how many different samples was each taxon found in?
asv <- otu_table(Df) %>% as("matrix") 
asv <- as.data.frame(asv) # convert to matrix before you can convert to data frame
...
tax_table(Df)[widespread_seq,] # this pull that row from ASV table

# what was most abundant (raw counts) taxon?
abund_seq <- which(otu_table(Df) %>% colSums() == max(otu_table(Df) %>% colSums()))
tax_table(Df)[abund_seq,]
otu_table(Df)[,abund_seq]
```
...
ps <- readRDS("./data/bact_and_fungi_clean_ps_object") # change to non-phylogeny stuff
...
ggsave("./output/16S_Kingdom-Level_Taxonomic_Proportions.png",dpi=500) # save figure for later use
...
saveRDS(seqs,"./data/16S_ASV_reference_sequences.RDS")

# Save RDS object for cleaned up Phyloseq object
saveRDS(ps, file = "./data/clean_phyloseq_object.RDS")
```
...
# Ordination
dca <- ordinate(ps_ra)
plot_ordination(ps_ra,dca,color = "Microbe") 
...
(
ord3 <- plot_ordination(ps_ra,unifrac,color = "Microbe",shape="Island") +
  geom_point(size=4) + theme_minimal() +
    theme(legend.position = "none") +
